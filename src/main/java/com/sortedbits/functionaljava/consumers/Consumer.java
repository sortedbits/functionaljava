...
public interface Consumer extends Arity {
    ...
    static <T, R> Consumer1<T> consumer1(Function1<T, R> f, java.util.function.Consumer<R> c) {
        return x -> c.accept(f.apply(x));
    }    

	static <T, R> java.util.function.Consumer<T> consumer(Function1<T, R> f, java.util.function.Consumer<R> c) {
        return x -> c.accept(f.apply(x));
    }
}
